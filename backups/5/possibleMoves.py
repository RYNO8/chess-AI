...
def possibleMoves(board, whiteMove, doKing=True, kingHasMoved=False):
    ...
    for pos in allPos:
        ...
        if idenPiece(cell, whiteMove): #your piece
            if cell.lower() == "r":
                moves[cell] = rook(board, cell, whiteMove)
            elif cell.lower() == "n":
                moves[cell] = knight(board, cell, whiteMove)
            elif cell.lower() == "b":
                moves[cell] = bishop(board, cell, whiteMove)
            elif cell.lower() == "q":
                moves[cell] = queen(board, cell, whiteMove)
            elif cell.lower() == "k" and doKing:
                moves[cell] = king(board, cell, whiteMove)
            elif cell.lower() == "p":
                moves[cell] = pawn(board, cell, whiteMove)
            else:
                break
            if moves[cell] == None:
                moves.pop(cell)
            elif oppKing in moves[cell]: #if can eat king, do that
    ...
    if not kingHasMoved and doKing: #castles
        ...
        if kingPos in moves:
            moves[kingPos].extend(castleMoves)
        else:
...
def threatening(board, pos):
    ...
    for pieceFunc, piece in [(pawn, 'p'), (knight, 'n'), (bishop, 'b'), (rook, 'r'), (queen, 'q'), (king, 'k')]:
        ...
        if pieceMoves:
            for endPos in pieceMoves:
                testCell = getCcell(board, endPos)
...
def rateMove(board, whiteMove, moveFrom, moveTo, tempboard):
    ...
    TRNASPOSITION = 0.1 # sort this out first (normalise tables) since king scores are higher than pawn scores
    ...
    # the moveTo creates a fork, of which you wil benefit
    ...
    rating -= pieceTables(board.getCell(moveFrom), moveFrom, whiteMove) * .1 #TRNASPOSITION
    rating += pieceTables(newBoard.getCell(moveTo), moveTo, whiteMove) * .1 #TRNASPOSITION
...
def runOptimise(func, *args):
    return multiprocessing.Process(target=func, args=tuple(args)).start()
